// frontend/src/app/emp/new/page.tsx
"use client";
...
export default function SimulationBuilderPage() {
  ...
  // ✅ Warn before closing/refreshing if unsaved changes exist
  useEffect(() => {
  ...
  // ✅ Check if simulation is ready for submission
  const isReadyToSubmit =
    ...
    variables.length > 0 &&
    decisions.length > 0 &&
    decisions.every((d: any) => (d.options || []).length === 6) &&
    decisions.every((d: any) => (d.options || []).every((o: any) => !!o.consequenceText)) &&
    decisions.every((d: any) => (d.options || []).every((o: any) => (o.impacts || []).length > 0));
  ...
  useEffect(() => {
    ...
    api.get("/emp/domains")
      .then(r => {
        console.log("🔍 Domains fetched:", r.data);
  ...
  const saveMeta = async () => {
    ...
    const duration = parseInt(meta.estimatedMinutes);
  ...
  const addVariable = async () => {
    ...
    finally { setAddingVariable(false); }
  };
  ...
  const addDecision = async () => {
    ...
    finally { setAddingDecision(false); }
  };
          ...
          <Section title="Simulation Details">
            <div className="grid sm:grid-cols-2 gap-4">
              <Field label="Domain" required>
                <select 
                  value={meta.domainId} 
                  onChange={e => { setIsDirty(true); setMeta(m => ({ ...m, domainId: e.target.value })); }}
                  disabled={!canEdit || domainsLoading}
                  className={inputCls}
                >
                  <option value="">
              ...
              <Field label="Title" required>
                <input value={meta.title} onChange={e => { setIsDirty(true); setMeta(m => ({ ...m, title: e.target.value })); }}
                  disabled={!canEdit} placeholder="e.g. The Founding PM" className={inputCls} maxLength={100}/>
                <p className="text-[10px] text-slate-400 mt-1 text-right">
                  {meta.title?.length || 0}/100
                </p>
              </Field>
            ...
            <div className="mt-4">
              <Field label="Description" required>
                <textarea value={meta.description} onChange={e => { setIsDirty(true); setMeta(m => ({ ...m, description: e.target.value })); }}
                  disabled={!canEdit} rows={3} placeholder="Brief simulation description…" className={textareaCls} maxLength={500}/>
                <p className="text-[10px] text-slate-400 mt-1 text-right">
                  {meta.description?.length || 0}/500
                </p>
              </Field>
            </div>
            <div className="mt-4">
              <Field label="Opening Scene Text">
                <textarea value={meta.openingScene || ""} onChange={e => { setIsDirty(true); setMeta(m => ({ ...m, openingScene: e.target.value })); }}
          ...
          <Section title="Company Background (Carousel Slide 1)">
            <div className="space-y-4">
              <Field label="Company Background" required>
                <textarea value={story.companyBackground} onChange={e => { setIsDirty(true); setStory(s => ({ ...s, companyBackground: e.target.value })); }}
          ...
          <Section title="Opening Scene (Shown before first decision)">
            <div className="space-y-4">
              ...
              <Field label="Senior Statement (Block 3)">
                <textarea value={story.seniorStatement || ""} onChange={e => { setIsDirty(true); setStory(s => ({ ...s, seniorStatement: e.target.value })); }}
          ...
          {canEdit && (
            <Section title="Add Variable">
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white ${addingVariable ? "opacity-50 cursor-not-allowed" : ""}`}
                style={{ background: G }}>
                {addingVariable ? <Loader2 className="w-4 h-4 animate-spin"/> : <Plus className="w-4 h-4"/>} 
                {addingVariable ? "Adding..." : "Add Variable"}
              </button>
            </Section>
          )}
        </div>
          ...
          {canEdit && (
            <Section title="Add Character">
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white ${addingCharacter ? "opacity-50 cursor-not-allowed" : ""}`}
                style={{ background: G }}>
                {addingCharacter ? <Loader2 className="w-4 h-4 animate-spin"/> : <Plus className="w-4 h-4"/>} 
          ...
          {canEdit && (
            <Section title="Add Decision">
              ...
              <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white ${addingDecision ? "opacity-50 cursor-not-allowed" : ""}`}
                style={{ background: G }}>
                {addingDecision ? <Loader2 className="w-4 h-4 animate-spin"/> : <Plus className="w-4 h-4"/>} 
        ...
        <div className="space-y-4">
          ...
          <Section title="Validation Checklist">
            <div className="space-y-2">
              {[
                ...
                { label: "All decisions have 6 options", ok: decisions.length > 0 ? decisions.every((d: any) => (d.options || []).length === 6) : null },
                { label: "All options have consequence", ok: decisions.length > 0 ? decisions.every((d: any) => (d.options || []).every((o: any) => !!o.consequenceText)) : null },
                { label: "All options have ≥1 impact",  ok: decisions.length > 0 ? decisions.every((d: any) => (d.options || []).every((o: any) => (o.impacts || []).length > 0)) : null },
              ].map(({ label, ok }) => (
                <div key={label} className="flex items-center gap-2.5">
                  ...
                  <span className={`text-sm ${ok === null ? "text-slate-400" : ok ? "text-slate-700" : "text-rose-700 font-medium"}`}>{label}</span>
                </div>
              ))}
            </div>
          </Section>
          ...
          {sim.status === "DRAFT" && !isAdmin && (
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold text-white transition-all w-full justify-center ${
                submitting || !isReadyToSubmit ? "opacity-50 cursor-not-allowed" : ""
              }`}
              style={{ background: isReadyToSubmit ? G : "#9ca3af" }}>
              {submitting ? (
                <Loader2 className="w-4 h-4 animate-spin"/>
              ) : (
                <>
                  {!isReadyToSubmit ? <AlertCircle className="w-4 h-4"/> : <Send className="w-4 h-4"/>}
                  {isReadyToSubmit ? "Submit for Review" : "Complete Checklist to Submit"}
                </>
              )}
            </button>
          )}
            ...
            <div className="border border-dashed border-slate-300 rounded-xl p-4 space-y-3">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea 
                    value={form.consequenceText} 
                    onChange={e => setForm(f => ({ ...f, consequenceText: e.target.value }))}
                    rows={2} 
                    placeholder="What happens as a result…" 
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs bg-white focus:outline-none resize-none" 
                    maxLength={300}
                  />
                  <p className="text-[10px] text-slate-400 mt-1 text-right">
                    {form.consequenceText?.length || 0}/300
                  </p>
                </div>
              </div>
